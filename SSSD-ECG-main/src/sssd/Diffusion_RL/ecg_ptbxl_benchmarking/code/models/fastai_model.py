...
# Script is in models/, so code/ is the parent directory
code_path = os.path.dirname(script_dir)
...
class fastai_model(ClassificationModel):
    def __init__(self,name,n_classes,freq,outputfolder,input_shape,pretrained=False,input_size=2.5,input_channels=12,chunkify_train=False,chunkify_valid=True,bs=128,ps_head=0.5,lin_ftrs_head=[128],wd=1e-2,epochs=50,lr=1e-2,kernel_size=5,loss="binary_cross_entropy",pretrainedfolder=None,n_classes_pretrained=None,gradual_unfreezing=True,discriminative_lrs=True,epochs_finetuning=30,early_stopping=None,aggregate_fn="max",concat_train_val=False,replace_output_layer=True,pretrained_file=None):
        ...
        self.replace_output_layer = replace_output_layer  # Whether to replace output layer when loading pretrained model
        self.pretrained_file = pretrained_file  # Direct path to pretrained model file (overrides pretrainedfolder search)
    ...
    def fit(self, X_train, y_train, X_val, y_val):
        ...
        if(self.pretrainedfolder is None): #from scratch
            ...
            if self.pretrained_file is not None:
                # Use directly specified file path
                pretrained_path = Path(self.pretrained_file)
                print(f"Loading pretrained model from specified file: {pretrained_path}")
                if pretrained_path.suffix == '.pkl':
                    # Load from pkl file
                    checkpoint = torch.load(pretrained_path, map_location='cpu')
                    # Handle different checkpoint formats
                    if isinstance(checkpoint, dict):
                        if 'model' in checkpoint:
                            # Fastai format: {'model': state_dict, ...}
                            state_dict = checkpoint['model']
                        elif 'model_state_dict' in checkpoint:
                            # PyTorch format: {'model_state_dict': state_dict, ...}
                ...
                elif pretrained_path.suffix == '.pth':
                    # Load from pth file using fastai's method
                    learn.path = pretrained_path.parent
                    learn.load(pretrained_path.stem)
                else:
                    raise ValueError(f"Unsupported pretrained file format: {pretrained_path.suffix}")
            # else:
            #     # Fallback to original search method
            #     # Check if pretrainedfolder contains a .pkl file (e.g., 100000.pkl)
            #     pkl_files = list(self.pretrainedfolder.glob('*.pkl'))
            #     if pkl_files:
            #         # Load from pkl file directly using torch.load
            #         pkl_path = pkl_files[0]  # Use the first pkl file found
            #         print(f"Loading pretrained model from pkl file: {pkl_path}")
            ...
            #         learn.model.load_state_dict(state_dict, strict=False)
            #     else:
            #         # Try fastai's default load method (expects .pth file)
            #         learn.path = self.pretrainedfolder
            #         learn.load(self.pretrainedfolder.stem)
            learn.path = self.outputfolder

            #exchange top layer (only if replace_output_layer is True)
            if self.replace_output_layer:
                ...
                learn.model.set_output_layer(output_layer_new)
                print(f"Replaced output layer from {output_layer.out_features} to {self.num_classes} classes")
            else:
                print(f"Keeping original output layer with {learn.model.get_output_layer().out_features} classes")
    ...
    def get_embedding(self, X):
        ...
        script_dir = os.path.dirname(os.path.abspath(__file__))
        # From models/ -> code/ -> ecg_ptbxl_benchmarking/ -> Diffusion_RL/ -> sssd/
        code_dir = os.path.dirname(script_dir)
        benchmark_dir = os.path.dirname(code_dir)
        diffusion_rl_dir = os.path.dirname(benchmark_dir)
        sssd_dir = os.path.dirname(diffusion_rl_dir)
        # Build absolute path
        model_path = os.path.join(sssd_dir, 'Diffusion_RL', 'ecg_ptbxl_benchmarking', 'output', 'exp_ICBEB_from_scatch', 'models', 'fastai_xresnet1d50', 'models', 'fastai_xresnet1d50.pth')
